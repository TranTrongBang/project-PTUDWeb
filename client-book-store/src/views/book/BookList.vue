            ...
            <div class="card">
                <div class="card-header">
                    <form class="d-flex justify-content-between">
                        ...
                        <button @click="bookList()" type="button" class="btn btn-outline-primary mt-3">
                            Tìm kiếm
                        </button>
                    </form>
                </div>
                <div class="card-header d-flex justify-content-end">
                    <router-link to="/sach/create" class="btn btn-outline-primary">Tạo mới Sách
                    </router-link>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-bordered" style='white-space: nowrap;'>
                        ...
                        <tbody>
                            ...
                            <tr v-for="book in books" :key="book._id">
                                ...
                                <td>{{ book.tacgia }}</td>
                                <td>
                                    <router-link :to="`/sach/detail/${book._id}`"
                                        class="btn btn-outline-info mx-1">Show</router-link>
                                    <router-link :to="`/sach/edit/${book._id}`"
                                        class="btn btn-outline-success mx-1">Edit</router-link>
                                    <button @click="handleDelete(book._id)" class="btn btn-outline-danger mx-1">
                                        Delete
                                    </button>
                                </td>
                            </tr>
        ...
        handleDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete(`http://localhost:3000/api/sachs/${id}`, { headers: authHeader() })
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
